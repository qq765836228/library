...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>修改图书</title>
		<meta name="keywords" content="修改图书" />
		<meta name="description" content="修改图书" />
	...
	$(function(){	
		/* $("#book_isbn").blur(function(){			
			...
			if(book_isbn != null && book_isbn != "" && book_isbn.trim() != oisbn.trim()){
				$.ajax({
		...
		$("#add").click(function(){
			...
			/* else if(book_isbn==null || book_isbn== ""){
				...
				$.ajax({
					url:"${pageContext.request.contextPath }/Book/editBook/"+page,
					data:{"book_id":book_id,"book_name":book_name,"category_id":category_id,"book_isbn":book_isbn,"book_autor":book_autor,"book_pubhouse":book_pubhouse,"book_page":book_page,"others":others},
					type:"post",
					dataType:"json",
					success:function(data){		
											...
											'<div class="fitting-mask3 " style="position: absolute;top: 35%;left: 40%;width: 400px;height: 200px;background-color:white;margin-top: -15px;margin-left: -15px;opacity:1;" id="choose">'+
												'<form style="border:1px solid red;width:400px;height:200px">'+
													...
													'<div class="sure" style="width:400px;height:96px">'+
														'<div align="center">'+
															'<div  class="btn btn-primary "  ><a href="${pageContext.request.contextPath }/Book/findAll/'+page+'" style="color: white;text-decoration:none;">返回</a></div>'+
														'</div>'+
													'</div>'+
												'</form>'+
											'</div>'+
						...
						<div class="row">
							...
							<form class="form-horizontal">
									<div class="form-group">
										<input type="hidden" id="book_id"  class="col-xs-10 col-sm-5"   name="book_id"  value="${book.book_id }"/>
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input  type="text" class="col-xs-6 col-sm-3" id="book_isbn" name="book_isbn" value="${book.book_isbn }" readonly="readonly"/>
											<span class="help-inline col-xs-12 col-sm-7">
												<label class="middle">
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<textarea rows="6" cols="50" id="others" name="others" >${book.others }</textarea>
										</div>
									</div>
