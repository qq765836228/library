...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title>借书登记</title>
		<meta name="keywords" content="借书登记" />
		<meta name="description" content="借书登记" />
				...
				if(book_isbn != null && book_isbn != ""){
					$.ajax({
						...
						success:function(data){	
							...
							if(data=="0"){
								$(".error").html("");
								$(".error").html("<font color='red'>此ISBN不存在或已被借出！</font>");
				...
				if(user_idcard != null && user_idcard != ""){
					$.ajax({
						...
						success:function(data){	
							...
							if(data=="0"){
								$(".error2").html("");
								$(".error2").html("<font color='red'>此用户不存在或者被停用！</font>");
								$("#add").attr("disabled","true");
							}
							return false;
						}
